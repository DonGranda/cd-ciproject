...
resource "aws_instance" "my-ec2" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      private_key = file("./projectkey.pem") # replace with your key-name 
      user        = "ubuntu"
      host        = self.public_ip
    }
    ...
    inline = [
      ...
      # Output
      "echo 'Access Jenkins Server here --> http://'$ip_addr':8080'",
      "echo 'Jenkins Initial Password: '$password''",
      "echo 'Access SonarQube Server here --> http://'$ip_addr':9000'",
